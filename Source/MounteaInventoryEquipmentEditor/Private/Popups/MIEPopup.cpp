...
void MIEPopup::OnBrowserLinkClicked(const FSlateHyperlinkRun::FMetadata& Metadata)
{
...
void MIEPopup::FormatChangelog(FString& InChangelog)
{
	FString WelcomeMessage = FString(R"(
<LargeText>Hello and thank you for using Mountea Inventory & Equipment!</>
...
The changelist is available publicly <a id="browser" href="https://github.com/Mountea-Framework/MounteaInventoryEquipment/blob/5.0/CHANGELOG.md">on our GitHub</>.
		)");
...
void MIEPopup::Register(const FString& Changelog)
{
	...
	UMIEPopupConfig* MIEPopupConfig = GetMutableDefault<UMIEPopupConfig>();

	if (FPaths::FileExists(UpdatedConfigFile))
	{
		MIEPopupConfig->LoadConfig(nullptr, *UpdatedConfigFile);
	}
	else
	{
		MIEPopupConfig->SaveConfig(CPF_Config, *UpdatedConfigFile);
	}
	...
	if (MIEPopupConfig->PluginVersionUpdate != CurrentPluginVersion)
	{
		MIEPopupConfig->PluginVersionUpdate = CurrentPluginVersion;
		MIEPopupConfig->SaveConfig(CPF_Config, *UpdatedConfigFile);
...
void MIEPopup::Open(const FString& Changelog)
{
	...
	TSharedPtr<SWindow> Window = SNew(SWindow)
				...
				.Title(FText::FromString("Mountea Inventory & Equipment"))
	...
	const TSharedRef<SVerticalBox> InnerContent = SNew(SVerticalBox)
		...
		[
			SNew(STextBlock)
			.Font(HeadingFont)
			.Text(FText::FromString("Mountea Inventory & Equipment"))
		...
		[
			...
			[
				...
				[
					...
					[
						...
						.Image(FMIEPopupStyle::GetBrush("MIEStyleSet.Icon.HeartIcon"))
					]
				]
			]
			...
			[
				...
				[
					...
					[
						SNew(SImage)
						.Image(FMIEPopupStyle::GetBrush("MIEStyleSet.Icon.MoneyIcon"))
					]
				]
			]
			+ SHorizontalBox::Slot().AutoWidth()
			...
			[
				...
				[
					...
					[
						SNew(SImage)
						.Image(FMIEPopupStyle::GetBrush("MIEStyleSet.Icon.SupportDiscord"))
					]
				]
			]
			+ SHorizontalBox::Slot().AutoWidth()
			...
			[
				...
				[
					...
					[
						SNew(SImage)
						.Image(FMIEPopupStyle::GetBrush("MIEStyleSet.Icon.UBIcon"))
					]
				]
			]
			+ SHorizontalBox::Slot().AutoWidth()
			...
			[
				...
				[
					...
					[
						SNew(SImage)
						.ColorAndOpacity(FLinearColor::Red)
						.Image(FMIEPopupStyle::GetBrush("MIEStyleSet.Icon.Close"))
					]
				]
			]
		];
