...
bool UMounteaInventoryComponent::AddItem_Implementation(const FInventoryItem& Item, bool bAutoStack)
{
   ...
   if (bAutoStack)
   {
       for (auto& ExistingItem : InventoryItems.Items)
       {
           if (ExistingItem.GetTemplate() == Item.GetTemplate())
           {
               return IncreaseItemQuantity(ExistingItem.GetGuid(), Item.GetQuantity());
           }
       }
   }
